...
@router.get("/doctors/paginated", response_model=List[Doctor])
def get_doctors_paginated(
    ...
    offset = (page - 1) * limit
    result = session.exec(select(Doctor).offset(offset).limit(limit)).all()
    return result

@router.get("/doctors/filter", response_model=List[Doctor])
def filter_doctors(
